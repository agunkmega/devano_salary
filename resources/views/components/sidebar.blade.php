...
<aside
    class="fixed inset-y-0 left-0 z-30 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 transform transition-all duration-300 ease-in-out flex flex-col"
    :class="[
...
>
    <div class="flex items-center h-16 px-4 border-b border-gray-200 dark:border-gray-700" :class="$store.sidebar.collapsed ? 'justify-center' : ''">
        ...
        <div class="flex items-center justify-center w-8 h-8 bg-blue-600 rounded-lg flex-shrink-0" x-show="$store.sidebar.collapsed">
            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z"/></svg>
        </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <div class="mb-4">
            <p class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500" x-show="!$store.sidebar.collapsed">{{ $groupName }}</p>
            <div class="space-y-1">
                @foreach($items as $item)
                    ...
                    <a
                        ...
                        x-bind:class="$store.sidebar.collapsed ? 'justify-center px-0' : ''"
                    >
...
<template x-teleport="body">
    <div x-show="$store.sidebar.mobileOpen" class="fixed inset-0 z-20 bg-black/50 lg:hidden" @click="$store.sidebar.toggleMobile()"></div>
</template>
