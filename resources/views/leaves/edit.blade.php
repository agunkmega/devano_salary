...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.leaves.update', $leave->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')

        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Form Edit</h3>
            <div class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <select name="leave_type_id" class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 @error('leave_type_id') border-red-500 @enderror">
                        ...
                        @foreach($leaveTypes as $lt)
                            <option value="{{ $lt->id }}" {{ old('leave_type_id', $leave->leave_type_id) == $lt->id ? 'selected' : '' }}>{{ $lt->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" x-model="endDate" @change="calcDays" value="{{ old('end_date', $leave->end_date->format('Y-m-d')) }}" class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 @error('end_date') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <textarea name="reason" rows="4" class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 @error('reason') border-red-500 @enderror" placeholder="Alasan pengajuan cuti/izin">{{ old('reason', $leave->reason) }}</textarea>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Biarkan kosong jika tidak ingin mengubah lampiran. PDF, JPG, PNG max 2MB</p>
                    @if($leave->attachment)
                        <p class="mt-1 text-xs text-emerald-600 dark:text-emerald-400">Lampiran saat ini: <a href="{{ asset('storage/' . $leave->attachment) }}" target="_blank" class="underline">Lihat</a></p>
                    @endif
...
<script>
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.data('employeeSearch', () => ({
            search: '{{ $leave->employee?->full_name ?? "" }}',
            open: false,
            selected: '{{ $leave->employee_id }}',
            selectedName: '{{ $leave->employee?->full_name ?? "" }}',
