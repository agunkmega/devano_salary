...
@section('page-subtitle', 'Kelola hari libur nasional')
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <th class="text-left py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">Nama Libur</th>
                        <th class="text-center py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">Agama</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        <td class="py-3 px-4 text-gray-900 dark:text-white font-medium">{{ $h->date->format('d/m/Y') }}</td>
                        <td class="py-3 px-4 text-gray-900 dark:text-white">{{ $h->name }}</td>
                        <td class="py-3 px-4 text-center">
                            @if($h->religion)
                            <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">{{ $h->religion }}</span>
                            @else
                            <span class="text-xs text-gray-400">Umum</span>
                            @endif
                        </td>
                        <td class="py-3 px-4 text-gray-500 dark:text-gray-400">{{ $h->description ?? '-' }}</td>
                        <td class="py-3 px-4 text-center">
                            @if($h->is_active)
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('admin.national-holidays.edit', $h->id) }}" class="p-2 text-gray-400 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                <form action="{{ route('admin.national-holidays.destroy', $h->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus libur ini?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="py-12 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada libur nasional</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
